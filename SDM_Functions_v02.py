...
def update_decision_node_options(model1, model2, decision_node_name):
    ...
    original_cpts = {child: model1.cpt(child).tolist() for child in children_of_decision_node}
...
def arc_transfer(model1, model2, node_name):
    model1_utility_node_name = next((node for node in model1.names() if model1.isUtilityNode(model1.idFromName(node))), None)
    model2_utility_node_name = next((node for node in model2.names() if model2.isUtilityNode(model2.idFromName(node))), None)
    ...
    model2_decision_node_name = next((node for node in model2.names() if model2.isDecisionNode(model2.idFromName(node))), None)
    model1_decision_node_name = next((node for node in model1.names() if model1.isDecisionNode(model1.idFromName(node))), None)
    ...
    for child_id in model2.children(model2_decision_node_id):
        ...
        if child_name in model1.names() and not model1.existsArc(model1_decision_node_id, model1.idFromName(child_name)):
    ...
    for parent_id in model2.parents(model2_decision_node_id):
        ...
        if parent_name in model1.names() and not model1.existsArc(model1.idFromName(parent_name), model1_decision_node_id):
    ...
    for common_parent_id in common_parents:
        ...
        if common_parent_name in model1.names():
            for parent_id in model1.parents(model1.idFromName(common_parent_name)):
                parent_name = model1.variable(parent_id).name()
                if model1.existsArc(model1.idFromName(parent_name), model1.idFromName(common_parent_name)):
                    model1.eraseArc(model1.idFromName(parent_name), model1.idFromName(common_parent_name))
    ...
    for node in model2.names():
        if model2.isChanceNode(model2.idFromName(node)) and node in model1.names():
            for parent_id in model2.parents(model2.idFromName(node)):
                parent_name = model2.variable(parent_id).name()
                if parent_name in model1.names() and not model1.existsArc(model1.idFromName(parent_name), model1.idFromName(node)):
                    model1.addArc(model1.idFromName(parent_name), model1.idFromName(node))

    if node_name in model2.names():
        node_id_model2 = model2.idFromName(node_name)
        for parent_id in model2.parents(node_id_model2):
            parent_name = model2.variable(parent_id).name()
            if parent_name in model1.names() and not model1.existsArc(model1.idFromName(parent_name), model1.idFromName(node_name)):
...
def arc_transfer(model1, model2, node_name):
    ...
    try:
        student_utility_node_name = next(node for node in model1.names() if model1.isUtilityNode(node))
        advisor_utility_node_name = next(node for node in model2.names() if model2.isUtilityNode(node))
    except StopIteration:
        print("No utility nodes found in one of the models. Arc transfer cannot proceed.")
        return model1
    ...
    try:
        advisor_decision_node = [node for node in model2.names() if model2.isDecisionNode(node)][0]
        student_decision_node = [node for node in model1.names() if model1.isDecisionNode(node)][0]
    except IndexError:
        print("No decision nodes found in one of the models. Arc transfer for decision nodes cannot proceed.")
        return model1

    for child_id in model2.children(advisor_decision_node):
        child_name = model2.variable(child_id).name()
        if child_name in model1.names() and not model1.existsArc(student_decision_node, child_name):
            model1.addArc(student_decision_node, child_name)

    for parent_id in model2.parents(advisor_decision_node):
        parent_name = model2.variable(parent_id).name()
        if parent_name in model1.names() and not model1.existsArc(parent_name, student_decision_node):
...
#                    model1.eraseArc(parent_name, common_parent_name)
    ...
    for node in model2.names():
        if model2.isChanceNode(node) and node in model1.names():
            for parent_id in model2.parents(model2.idFromName(node)):
                parent_name = model2.variable(parent_id).name()
                if parent_name in model1.names() and not model1.existsArc(parent_name, node):
    ...
    if model2.isChanceNode(node_name):
        ...
        if child_ids_model2:
            ...
            if model2.isUtilityNode(child_name_model2):
                utility_nodes_model1 = [node for node in model1.names() if model1.isUtilityNode(node)]
                if utility_nodes_model1:
                    ...
                    if not model1.existsArc(node_name, child_name_model1):
                        ...
                        print(f"Arc added between {node_name} and {child_name_model1} in model1.")
                else:
...
def add_chance_related_arcs_from_advisor_to_student(advisor_model, student_model):
    ...
    advisor_utility_parents = set(advisor_model.parents(advisor_model.idFromName(advisor_utility_node)))
    student_utility_parents = set(student_model.parents(student_model.idFromName(student_utility_node)))
    common_parents = advisor_utility_parents & student_utility_parents
